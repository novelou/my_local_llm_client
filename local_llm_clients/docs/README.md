...
会話履歴は `local_llm_clients/sessions/` 以下に JSON として保存されます。Codex CLI 風に、同じ会話をロードして続きから作業できます。
...
- `local_llm_clients/sessions/agent/`: local file agent の会話履歴
- `local_llm_clients/sessions/mcp/`: Unity MCP client の会話履歴
>>>>>>> theirs
